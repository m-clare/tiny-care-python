...
def horizontal_bar_labels(categories):
    ...
    subplots = make_subplots(
        ...
        cols=1,
        subplot_titles=[
            "<b>{0} - {1}</b>".format(x["name"], x["value"]) for x in categories
        ],
        shared_xaxes=True,
        print_grid=False,
    ...
    subplots.update_layout(font_family="Dank Mono, Consolas, monospace")
    ...
    for x in subplots["layout"]["annotations"]:
        x["font"] = dict(size=20, color="black")
...
if __name__ == " __main__":
    ...
    for i in range(len(text)):
        ...
        subplots.show()
        subplots.write_image("./assets/org.png", format="png", width=150, height=300)
